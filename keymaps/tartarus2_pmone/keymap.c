...
// USB personality, cached: the USB stack asks for descriptors before the keymap
// runs, so this must be readable without touching EEPROM. 0 = PowerMic identity
// (dictation) is the safe default, so a fresh board works out of the box.
static uint8_t pm_via_mode = 0;

// v4: v3 blocks in the field were written by an early mode-toggle bug with every
// colour and the brightness zeroed - loading one turns the LEDs black. Bumping
// the version invalidates them so the compiled defaults (light blue / red /
// yellow, full brightness) take over.
#define TRGB_CFG_VERSION 4
...
typedef struct PACKED {
    ...
    uint8_t via_mode;  // 0 = PowerMic identity (dictation), 1 = VIA identity
} trgb_settings_t;
...
void powermic_mode_toggle(void);  // defined at the end of this file

bool process_record_user(uint16_t keycode, keyrecord_t *record) {

    key_event_count++;  // input-path diagnostic (0xD4)

    // USB personality chord: hold the THUMB key and tap key 01 to swap between
    // PowerMic mode (dictation) and VIA mode (reprogramming). Two keys at
    // opposite ends of the pad, so it cannot be struck by accident in normal use.
    {
        static bool thumb_down = false;
        if (record->event.key.row == 28 && record->event.key.col == 2) {
            thumb_down = record->event.pressed;
        } else if (thumb_down && record->event.pressed &&
                   record->event.key.row == 3 && record->event.key.col == 6) {
            powermic_mode_toggle();  // saves the flag and reboots; never returns
            return false;
        }
    }
...
// --- USB personality switch (PowerMic mode <-> VIA mode) --------------------
// One image, two identities. PowerMic mode presents 0554:1001 with raw HID
// omitted, so PowerScribe sees exactly one usbdevPowerMic2 and binds it; VIA
// mode presents the board's own ID with raw HID, so PowerScribe sees no
// candidate at all and VIA can connect. The mode lives in the keyboard's
// eeconfig word, so it survives reboots and reflashes.
//
// Toggle: hold the THUMB key and tap key 01, then replug (the switch forces a
// USB re-enumeration, but Windows is happiest with a physical replug after an
// identity change). Deliberately awkward so it cannot be hit by accident.

// Runs before the USB stack starts (keyboard_setup precedes protocol_init), so
// the stored identity is known by the time descriptors are requested. Reading it
// in keyboard_post_init_user is too late - enumeration has already happened and
// the board would always come up in the default identity.
void keyboard_pre_init_user(void) {
    trgb_settings_t s;
    eeconfig_read_kb_datablock(&s);
    if (s.version == TRGB_CFG_VERSION) {
        pm_via_mode = s.via_mode;
    }
}

bool powermic_mode_active(void) {
    return pm_via_mode == 0;
}

void powermic_mode_toggle(void) {
    trgb_settings_t s;
    eeconfig_read_kb_datablock(&s);
    if (s.version != TRGB_CFG_VERSION) {
        s = (trgb_settings_t){0};
        s.version = TRGB_CFG_VERSION;
    }
    s.via_mode = pm_via_mode ? 0 : 1;
    eeconfig_update_kb_datablock(&s);
    soft_reset_keyboard();  // reboot re-enumerates under the other identity
}
