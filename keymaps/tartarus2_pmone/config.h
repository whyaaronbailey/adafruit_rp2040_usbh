...
// Dual-personality: the COMPILED device identity is the board's own 239A:0001
// (VIA mode). PowerMic mode swaps in PowerMicDeviceDescriptor (0554:1001) at
// runtime. The two modes MUST use different VID/PID: Windows caches a device's
// interface layout per VID/PID, so presenting a different interface count under
// one ID gives stale bindings and partial enumeration (only MI_00 came up) -
// which is exactly what a same-0554 dual build did.
//
// Strings stay Nuance/PowerMicII-NS so PowerMic mode is faithful; VIA matches on
// VID/PID, not strings, so it is harmless there.
#undef VENDOR_ID
#undef PRODUCT_ID
#undef DEVICE_VER
#undef MANUFACTURER
...
// (POWERMIC_FORCE_EP1 removed: it forces endpoint renumbering that complicates
// the dual-mode splice; the endpoint address does not affect recognition.)

// A serial number makes each USB personality a distinct (VID,PID,serial) device
// to the host, so Windows never serves one mode's cached interface layout for
// the other (that showed up as Code 10 / "Error" on an interface after a mode
